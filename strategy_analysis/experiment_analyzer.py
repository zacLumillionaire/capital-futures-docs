...
class ExperimentAnalyzer:
    ...
    def load_results_dataframe(self, success_only: bool = True) -> pd.DataFrame:
        """載入實驗結果為 DataFrame"""
    ...
    def get_top_results(self, metric: str = "total_pnl", top_n: int = 10, ascending: bool = False) -> pd.DataFrame:
        """獲取最佳結果"""
        df = self.load_results_dataframe(success_only=True)
    ...
    def analyze_parameter_sensitivity(self, target_metric: str = "total_pnl") -> Dict[str, float]:
        """分析參數敏感度"""
        df = self.load_results_dataframe(success_only=True)
    ...
    def find_optimal_parameters(self, metric: str = "total_pnl") -> Dict[str, Any]:
        """找到最佳參數組合"""
        df = self.load_results_dataframe(success_only=True)
    ...
    def generate_analysis_report(self, output_file: str = None) -> str:
        ...
        top_results = self.get_top_results("total_pnl", 10)
        optimal_params = self.find_optimal_parameters("total_pnl")
        sensitivity = self.analyze_parameter_sensitivity()
