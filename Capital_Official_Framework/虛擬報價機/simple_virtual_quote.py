# 簡化版虛擬報價機
# Simple Virtual Quote Machine - 核心功能版本
...
class SimpleVirtualQuote:
    ...
    def start_quote_feed(self):
        ...
        print("🚀 [SimpleVirtual] 報價推送已啟動")
    
    def stop_quote_feed(self):
        """停止報價推送"""
        self.running = False
        print("🛑 [SimpleVirtual] 報價推送已停止")
    ...
    def _quote_loop(self):
        ...
        while self.running:
            try:
                ...
                print(f"❌ [SimpleVirtual] 報價循環錯誤: {e}")
                time.sleep(0.1)
    ...
    def process_order(self, user_id, async_flag, order_obj):
        ...
        try:
            ...
            print(f"📋 [SimpleVirtual] 收到下單: {order_id} - {'買進' if buy_sell == 0 else '賣出'} {product} {quantity}口 @{price}")
            ...
            print(f"❌ [SimpleVirtual] 下單處理失敗: {e}")
            return ("", -1)
    ...
    def _process_fill(self, order_id, account, product, buy_sell, price, quantity):
        ...
        try:
            ...
            if random.random() < 0.95:
                ...
                print(f"✅ [SimpleVirtual] 訂單成交: {order_id} @ {fill_price}")
            else:
                # 取消
                time.sleep(0.15)
                self._send_reply(order_id, account, product, buy_sell, price, quantity, "C", 0, 0)
                print(f"❌ [SimpleVirtual] 訂單取消: {order_id}")
                
        except Exception as e:
            print(f"❌ [SimpleVirtual] 成交處理錯誤: {e}")
...
class MockSKCenterLib:
    def SKCenterLib_Login(self, user_id, password):
        print(f"🎯 [SimpleVirtual] 模擬登入: {user_id}")
        return 0
...
class MockSKOrderLib:
    def SKOrderLib_Initialize(self):
        print("🎯 [SimpleVirtual] 模擬下單初始化")
        return 0
    
    def ReadCertByID(self, user_id):
        print(f"🎯 [SimpleVirtual] 模擬憑證讀取: {user_id}")
        return 0
...
def SetID(id):
    global Global_IID
    Global_IID = id
    print(f"🎯 [SimpleVirtual] SetID: {Global_IID}")
...
print("📦 [SimpleVirtual] 簡化版虛擬報價機載入完成")
print("🎯 [SimpleVirtual] 核心功能: 報價推送 + 下單成交模擬")
