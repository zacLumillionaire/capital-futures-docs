# 實驗版回測_Profit-Funded Risk_多口.py
import logging
import sys
import os
...
try:
    ...
    # 實驗模式：創建簡化的替代函數
    def init_all_db_pools():
        logger.info("🧪 實驗模式：跳過資料庫連線池初始化")
        pass
    ...
    try:
        ...
        class MockSQLiteConnection:
            def init_sqlite_connection(self):
                logger.info("🧪 實驗模式：跳過SQLite連接初始化")
                pass
        sqlite_connection = MockSQLiteConnection()
...
def _run_multi_lot_logic(day_session_candles: list, trade_candles: list, config: StrategyConfig, range_high, range_low) -> tuple[Decimal, str]:
    ...
    for exit_candle in trade_candles[entry_candle_index + 1:]:
        ...
        for lot in lots:
            ...
            if hasattr(config, 'experiment_mode') and getattr(config, 'experiment_mode', False) and hasattr(config, 'experiment_take_profit_points'):
                ...
                if position == 'LONG':
                    ...
                    exit_price = target_price if tp_triggered else None
                else:  # SHORT
                    target_price = entry_price - take_profit_points
                    tp_triggered = exit_candle['low_price'] <= target_price
                    exit_price = target_price if tp_triggered else None
            else:
                # 原始模式：使用區間邊緣作為停利點
                if position == 'LONG':
                    tp_triggered = exit_candle['high_price'] >= range_high  # 原策略SHORT的停損點
                    exit_price = range_high if tp_triggered else None
                else:  # SHORT
                    tp_triggered = exit_candle['low_price'] <= range_low   # 原策略LONG的停損點
                    exit_price = range_low if tp_triggered else None

            if tp_triggered:
                if position == 'LONG':
                    lot['pnl'] = exit_price - entry_price
                else:  # SHORT
                    lot['pnl'] = entry_price - exit_price
                ...
                # 根據實際損益顯示停利或停損
                if lot['pnl'] > 0:
                    logger.info(f"  ✅ 第{lot['id']}口觸及停利點 | 時間: {current_time}, 出場價: {int(round(exit_price))}, 損益: {int(round(lot['pnl'])):+d}")
                else:
                    logger.info(f"  ❌ 第{lot['id']}口觸及停損點 | 時間: {current_time}, 出場價: {int(round(exit_price))}, 損益: {int(round(lot['pnl'])):+d}")
        ...
        for lot in lots:
            ...
            #         lot['pnl'], lot['status'], exited_by_sl = rule.fixed_tp_points, 'exited', True
            #         logger.info(f"  ✅ 第{lot['id']}口固定停利 | 時間: {current_time}, 損益: {int(round(lot['pnl'])):+d}")
...
def main():
    ...
    # 添加命令行參數支持
    parser = argparse.ArgumentParser(description='Profit-Funded Risk 多口交易策略回測 - 實驗版')
    ...
    parser.add_argument('--experiment-mode', action='store_true', help='實驗模式')
    parser.add_argument('--stop-loss-points', type=int, help='實驗用固定停損點數')
    parser.add_argument('--take-profit-points', type=int, help='實驗用固定停利點數')
    ...
    if args.gui_mode and args.config:
        try:
            ...
            result = run_backtest(strategy_config, start_date, end_date, True, range_start_time, range_end_time)

            # 輸出 JSON 格式結果供 enhanced_mdd_optimizer.py 解析
            if result:
                import json
