            ...
            try:
                ...
                if '🚨' in line and '口風險平倉' in line:
                    ...
                    events.append({
                        ...
                        'entry_price': current_entry_price if current_entry_price is not None else 'N/A',
...
def calculate_enhanced_statistics(daily_df: pd.DataFrame, events_df: pd.DataFrame) -> Dict[str, Any]:
    ...
    if not events_df.empty:
        for lot_num in sorted(events_df['lot_number'].unique()):
            if pd.isna(lot_num):
                continue
            lot_data = events_df[events_df['lot_number'] == lot_num]
            winning_trades = len(lot_data[lot_data['pnl'] > 0])
            losing_trades = len(lot_data[lot_data['pnl'] < 0])
            total_trades = len(lot_data)
            ...
            lot_analysis[f'lot_{int(lot_num)}'] = {
                ...
                'win_rate': (winning_trades / total_trades * 100) if total_trades > 0 else 0,
                'avg_pnl': lot_data['pnl'].mean() if total_trades > 0 else 0,
                'max_win': lot_data['pnl'].max() if total_trades > 0 else 0,
                'max_loss': lot_data['pnl'].min() if total_trades > 0 else 0,
                'pnl_in_nt': lot_data['pnl'].sum() * 50  # 台指期貨 1點 = NT$50
            }
...
def generate_enhanced_html_report(statistics: Dict[str, Any], chart_data: Dict[str, str], config_data: Dict[str, Any], events_df: pd.DataFrame, strategy_summary: str = "", log_content: str = "") -> str:
    ...
    if lot_analysis:
        # 計算組合績效比較
        lot1_pnl = lot_analysis.get('lot_1', {}).get('total_pnl', 0)
        lot2_pnl = lot_analysis.get('lot_2', {}).get('total_pnl', 0)
        lot3_pnl = lot_analysis.get('lot_3', {}).get('total_pnl', 0)

        combo_2lots = lot1_pnl + lot2_pnl
        combo_3lots = lot1_pnl + lot2_pnl + lot3_pnl

        lot_table_html = f"""
        <div class="table-container">
            <h3>📋 Lot Performance Analysis & Strategy Comparison</h3>

            <!-- 個別口數績效表 -->
            <h4>🎯 Individual Lot Performance</h4>
            <table class="performance-table">
                <thead>
                    <tr>
                        <th>Lot</th>
                        <th>Total P&L (Points)</th>
                        <th>Total P&L (NT$)</th>
                        <th>Trades</th>
                        <th>Win/Loss</th>
                        <th>Win Rate</th>
        ...
        for lot_key, lot_data in sorted(lot_analysis.items()):
            ...
            nt_class = 'positive' if lot_data['pnl_in_nt'] > 0 else 'negative' if lot_data['pnl_in_nt'] < 0 else 'neutral'

            lot_table_html += f"""
                    <tr>
                        <td><strong>第{lot_num}口</strong></td>
                        <td class="{pnl_class}"><strong>{lot_data['total_pnl']:+.1f}</strong></td>
                        <td class="{nt_class}"><strong>NT${lot_data['pnl_in_nt']:+,.0f}</strong></td>
                        <td>{lot_data['trades']}</td>
                        <td>{lot_data['winning_trades']}/{lot_data['losing_trades']}</td>
                        <td>{lot_data['win_rate']:.1f}%</td>
                        <td class="{pnl_class}">{lot_data['avg_pnl']:+.1f}</td>
                        <td class="positive">{lot_data['max_win']:+.0f}</td>
                        <td class="negative">{lot_data['max_loss']:+.0f}</td>
                    </tr>
            """

        # 添加組合策略比較表
        combo_2lots_class = 'positive' if combo_2lots > 0 else 'negative' if combo_2lots < 0 else 'neutral'
        combo_3lots_class = 'positive' if combo_3lots > 0 else 'negative' if combo_3lots < 0 else 'neutral'
        ...
        lot_table_html += f"""
            ...
            <!-- 策略組合比較表 -->
            <h4>🔄 Strategy Combination Comparison</h4>
            <table class="performance-table">
                <thead>
                    <tr>
                        <th>Strategy</th>
                        <th>Total P&L (Points)</th>
                        <th>Total P&L (NT$)</th>
                        <th>Performance vs 3-Lot</th>
                        <th>Recommendation</th>
                    </tr>
                </thead>
                <tbody>
                    <tr>
                        <td><strong>🎯 2-Lot Strategy</strong><br><small>(第1口 + 第2口)</small></td>
                        <td class="{combo_2lots_class}"><strong>{combo_2lots:+.1f}</strong></td>
                        <td class="{combo_2lots_class}"><strong>NT${combo_2lots * 50:+,.0f}</strong></td>
                        <td class="{'positive' if combo_2lots > combo_3lots else 'negative' if combo_2lots < combo_3lots else 'neutral'}">
                            {combo_2lots - combo_3lots:+.1f} points
                        </td>
                        <td>{'✅ 建議使用' if combo_2lots > combo_3lots else '❌ 不建議' if combo_2lots < combo_3lots else '⚖️ 相同績效'}</td>
                    </tr>
                    <tr>
                        <td><strong>🎯 3-Lot Strategy</strong><br><small>(第1口 + 第2口 + 第3口)</small></td>
                        <td class="{combo_3lots_class}"><strong>{combo_3lots:+.1f}</strong></td>
                        <td class="{combo_3lots_class}"><strong>NT${combo_3lots * 50:+,.0f}</strong></td>
                        <td class="neutral">基準</td>
                        <td>{'✅ 建議使用' if combo_3lots >= combo_2lots else '❌ 不建議'}</td>
                    </tr>
                </tbody>
            </table>
            ...
            <div class="analysis-conclusion">
                ...
                <div class="conclusion-box {'positive' if combo_2lots > combo_3lots else 'negative' if combo_2lots < combo_3lots else 'neutral'}">
        """

        if combo_2lots > combo_3lots:
            improvement = combo_2lots - combo_3lots
            lot_table_html += f"""
                    <p><strong>🎉 建議使用 2-Lot 策略</strong></p>
                    <p>• 2-Lot策略比3-Lot策略多獲利 <strong>{improvement:+.1f} 點</strong> (NT${improvement * 50:+,.0f})</p>
                    <p>• 第3口的貢獻為負值，建議移除以提升整體績效</p>
                    <p>• 降低交易成本和風險暴露的同時提升獲利能力</p>
            """
        elif combo_3lots > combo_2lots:
            improvement = combo_3lots - combo_2lots
            lot_table_html += f"""
                    <p><strong>🎯 建議使用 3-Lot 策略</strong></p>
                    <p>• 3-Lot策略比2-Lot策略多獲利 <strong>{improvement:+.1f} 點</strong> (NT${improvement * 50:+,.0f})</p>
                    <p>• 第3口提供正向貢獻，建議保留以最大化獲利</p>
                    <p>• 雖然增加風險暴露，但整體獲利能力更強</p>
            """
        else:
            lot_table_html += f"""
                    <p><strong>⚖️ 兩種策略績效相同</strong></p>
                    <p>• 2-Lot和3-Lot策略獲利相同</p>
                    <p>• 建議選擇2-Lot策略以降低風險暴露和交易成本</p>
                    <p>• 第3口對整體績效無貢獻</p>
            """
    ...
    if not events_df.empty:
        ...
        for _, trade in events_df.iterrows():
            ...
            # 處理entry_price顯示
            entry_price_display = trade['entry_price']
            if pd.isna(entry_price_display) or entry_price_display == 'N/A':
                entry_price_display = 'N/A'
            else:
                entry_price_display = f"{int(entry_price_display)}"
